...
(function(root, factory) {
	...
	var Sifter = function(items, settings) {
		this.items = items;
		this.settings = settings || { diacritics: true, stickies: [] };
	};
	...
	Sifter.prototype.search = function(query, options) {
		...
		var stickies = self.settings.stickies;
		for (var i  = stickies.length - 1; i >= 0; i--) {
			if (stickies[i] in self.items) {
				search.items.unshift({ score: 1, id: stickies[i] });
			}
		}
...
(function(root, factory) {
	...
	var Selectize = function($input, settings) {
		...
		// search system
		var sOpts = { diacritics: settings.diacritics, stickies: settings.stickies };
		self.sifter = new Sifter(this.options, sOpts);
	...
	$.extend(Selectize.prototype, {
		...
		refreshOptions: function(triggerDropdown) {
			...
			// console.log('refreshOptions', self);
			// build markup
